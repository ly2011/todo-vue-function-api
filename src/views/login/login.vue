  ...
  <section class="login">
    <div class="box">
      <div class="input last">
        <input type="text" maxlength="50" v-model.trim="accessToken" placeholder="Access Token" />
      </div>
...
export default {
  name: 'Login',
  setup (props, ctx) {
    const accessToken = value(window.localStorage.save_access_token || '393d3e5b-efad-4800-8326-355a52a54eae')
    ...
    const handleLogin = () => {
      ...
      serviceLogin({ accesstoken: accessToken.value }).then(res => {
        if (res.success) {
          login({
